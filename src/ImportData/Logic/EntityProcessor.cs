...
namespace ImportData
{
    public class EntityProcessor
    {
        public static void Procces(Type type, string xlsxPath, string sheetName, Dictionary<string, string> extraParameters, string searchDoubles, Logger logger)
        {
            ...
            var templateEntity = (Entity)getEntity.Invoke(processor, new object[] { new string[0], extraParameters });
            var expectedParamsCount = templateEntity.GetPropertiesCount(); // для Company будет 13
            const int serviceCols = 3;
            ...
            foreach (var importItem in importData.Skip(1))
            {
                ...
                // Если в конце реально есть 3 служебные колонки (итог/дата/подробности) — не берём их в параметры сущности.
                // Если их нет — берём все входные колонки как есть.
                int takeCount;
                if (countItem >= expectedParamsCount + serviceCols)
                    takeCount = expectedParamsCount;
                else
                    takeCount = Math.Min(countItem, expectedParamsCount);

                foreach (var data in importItem.Take(takeCount))
                    arrayItems.Add(data);
                ...
                if (entity != null)
                {
                    ...
                    else
                    {
                        var message = string.Format("Количества входных параметров недостаточно. " +
                          "Количество ожидаемых параметров {0}. Количество переданных параметров {1}.", entity.GetPropertiesCount(), importItem.Length);
            ...
            string[] text = new string[] { "Підсумок", "Дата", "Подробиці" };
            for (int i = 1; i <= 3; i++)
            {
            ...
            foreach (var result in listResult)
            {
                if (result.Where(x => x.ErrorType == Constants.ErrorTypes.Error).Any())
                {
                    ...
                    text = null;
                    text = new string[] { "Не завантажено", DateTime.Now.ToString("d"), message };
                    for (int i = 1; i <= 3; i++)
                    {
                        var title = excelProcessor.GetExcelColumnName(paramCount + i);
                ...
                else if (result.Where(x => x.ErrorType == Constants.ErrorTypes.Warn).Any())
                {
                    ...
                    text = null;
                    text = new string[] { "Завантажено частково", DateTime.Now.ToString("d"), message };
                    for (int i = 1; i <= 3; i++)
                    {
                        var title = excelProcessor.GetExcelColumnName(paramCount + i);
                        var arrayParams = new ArrayList { text[i - 1], title, row };
                ...
                else
                {
                    // TODO: Добавить локализацию строки.
                    text = null;
                    text = new string[] { "Завантажено", DateTime.Now.ToString("d"), string.Empty };
                    for (int i = 1; i <= 3; i++)
                    {
                        var title = excelProcessor.GetExcelColumnName(paramCount + i);
                        var arrayParams = new ArrayList { text[i - 1], title, row };
