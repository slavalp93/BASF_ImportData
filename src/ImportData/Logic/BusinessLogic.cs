...
namespace ImportData
{
    public class BusinessLogic
    {
            ...
            Dictionary<string, Sungero.Core.Enumeration?> sexProperty = new Dictionary<string, Sungero.Core.Enumeration?>
      {
        ...
        /// <summary>
        /// Получение организацию по ЄДРПОУ.
        /// </summary>
        /// <param name="session">Текущая сессия.</param>
        /// <param name="edrpou">ЄДРПОУ.</param>
        /// <param name="exceptionList">Список ошибок.</param>
        /// <param name="logger">Логировщик.</param>
        /// <returns>Контрагент.</returns>
        public static litiko.BASF.ICompany GetCompanyByEDRPOU(Session session, string edrpou, List<Structures.ExceptionsStruct> exceptionList, NLog.Logger logger)
        {
            var companies = Enumerable.ToList(session.GetAll<litiko.BASF.ICompany>().Where(x => x.EGRPOUlitiko == edrpou));
            var company = (Enumerable.FirstOrDefault<litiko.BASF.ICompany>(companies));
            if (companies.Count > 1)
            {
                var message = string.Format("Знайдено кілька контрагентів з ЄДРПОУ \"{0}\". Перевірте, чи вибрано правильний запис.", edrpou);
                exceptionList.Add(new Structures.ExceptionsStruct { ErrorType = Constants.ErrorTypes.Warn, Message = message });
                logger.Warn(message);
            }
            return company;
        ...
        /// <param name="nameEN">Наименование (англ).</param>
        /// <param name="exceptionList">Список ошибок.</param>
        /// <param name="logger">Логировщик.</param>
        /// <returns>Организация.</returns>
        public static litiko.BASF.ICompany CreateCompany(Session session, string edrpou, string name, string nameEN, List<Structures.ExceptionsStruct> exceptionList, NLog.Logger logger)
        {
            var company = litiko.BASF.Companies.Null;
            try
            {
                company = session.Create<litiko.BASF.ICompany>();
                company.EGRPOUlitiko = edrpou;
                company.Name = name;
                company.LegalName = name;
                company.NameENlitiko = nameEN;
                company.LegalNameENlitiko = nameEN;
                company.Save();
            }
            catch (Exception ex)
            {
                var message = string.Format("Не вдалося створити організацію з ЄДРПОУ \"{0}\". Текст помилки: {1}.", edrpou, ex.Message);
                exceptionList.Add(new Structures.ExceptionsStruct { ErrorType = Constants.ErrorTypes.Warn, Message = message });
                logger.Warn(message);
            }
        ...
        public static Sungero.Parties.IBank GetBank(Session session, string name, List<Structures.ExceptionsStruct> exceptionList, NLog.Logger logger)
        {
            ...
            if (banks.Count > 1)
            {
                var message = string.Format("Знайдено кілька банків із найменуванням \"{0}\". Перевірте, чи вибрано правильний запис.", name);
                exceptionList.Add(new Structures.ExceptionsStruct { ErrorType = Constants.ErrorTypes.Warn, Message = message });
                logger.Warn(message);
            }
            return bank;
        ...
        /// <param name="contactFIO">ФИО.</param>
        /// <param name="exceptionList">Список ошибок.</param>
        /// <param name="logger">Логировщик.</param>
        /// <returns>Банк.</returns>
        public static litiko.BASF.IContact GetContact(Session session, litiko.BASF.ICompany company,  string contactFIO, List<Structures.ExceptionsStruct> exceptionList, NLog.Logger logger)
        {
            var contacts = Enumerable.ToList(session.GetAll<litiko.BASF.IContact>().Where(x => x.Company.Id == company.Id && x.Name == contactFIO));
            var contact = (Enumerable.FirstOrDefault<litiko.BASF.IContact>(contacts));
            if (contacts.Count > 1)
            {
                var message = string.Format("Знайдено кілька контактів по організації \"{0}\" із найменуванням \"{1}\". Перевірте, чи вибрано правильний запис.", company.Name, contactFIO);
                exceptionList.Add(new Structures.ExceptionsStruct { ErrorType = Constants.ErrorTypes.Warn, Message = message });
                logger.Warn(message);
            }
            return contact;
        ...
        /// <param name="logger">Логировщик.</param>
        /// <returns>Организация.</returns>
        public static litiko.BASF.IContact CreateContact(Session session, litiko.BASF.ICompany company, string contactFIO, List<Structures.ExceptionsStruct> exceptionList, NLog.Logger logger)
        {
            var contact = litiko.BASF.Contacts.Null;
            try
            {
                contact = session.Create<litiko.BASF.IContact>();
                contact.Company = company;
                contact.Name = contactFIO;
                contact.Save();
            }
            catch (Exception ex)
            {
                var message = string.Format("Не вдалося створити контакт \"{0}\" для організації \"{1}\". Текст помилки: {1}.", contactFIO, company.Name, ex.Message);
                exceptionList.Add(new Structures.ExceptionsStruct { ErrorType = Constants.ErrorTypes.Warn, Message = message });
                logger.Warn(message);
            }
